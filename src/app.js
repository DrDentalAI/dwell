...
renderPricing() {
  ...
  inner += `<label class="fl">ZIP / postal code (optional)</label>
    <input type="text" inputmode="text" placeholder="48307" value="${h(S.placeRaw || '')}"
           oninput="UI.setPlace(this.value)">
    <p class="hint">${
      !S.placeRaw ? 'Only a few networks price differently by state or province. This picks the right one when they do.'
      : pl ? `Reading this as <b>${h(pl.state || pl.province)}</b>, ${pl.country === 'US' ? 'USA' : 'Canada'}. ` +
      ...
      : 'Not a ZIP or postal code I recognise — nothing has been applied.'}</p>`;
...
renderFinder() {
  ...
  const liveRow = (r, i) => `<button class="row-btn" onclick="UI.pickLive(${i})">
      <div style="flex:1;min-width:0">
  ...
  box.innerHTML = `
    ...
    <input type="search" id="finder-q" placeholder="Meijer, Rochester, EVgo\u2026" value="${h(q)}"
           oninput="UI.finderSearch(this.value)">

    <div class="rows" style="margin-top:10px">
      ${sites.length ? sites.map(siteRow).join('')
        : `<p class="hint">Nothing in the built-in list matches. Try the live lookup below.</p>`}
    </div>
    <p class="hint">Built-in list \u2014 ${L.KNOWN_SITES.length} sites checked stall-by-stall against the
      operator's own pages. Works with no signal. Metro Detroit for now.</p>

    <div class="divider"></div>
    <button class="btn sm" style="width:100%" onclick="UI.findNearby()"
      ${S.finder.busy ? 'disabled' : ''}>${S.finder.busy ? 'Searching\u2026' : '\u{1F4CD} Search near me (needs signal)'}</button>
    ${S.finder.error ? `<div class="note bad" style="margin-top:10px">${h(S.finder.error)}</div>` : ''}
    ${live.length ? `<div class="rows" style="margin-top:10px">${live.map(liveRow).join('')}</div>
      <p class="hint">Live results from the US Department of Energy's station database.
        It publishes hardware, not prices \u2014 no free source publishes live per-station rates,
        so set the price yourself once and it sticks.</p>` : ''}`;
},
...
setNrelKey(v) { S.nrelKey = String(v || '').trim() || null; save(); },
finderSearch(v) { S.finder.q = v; this.renderFinder(); const el = $('finder-q'); if (el) { el.focus(); el.setSelectionRange(v.length, v.length); } },
...
async findNearby() {
  ...
  /* HOST CHANGED 2026. DOE renamed NREL to the National Laboratory of the
     Rockies; the developer network moved to developer.nlr.gov and DNS for
     nrel.gov and every subdomain "ceased to resolve and will not redirect".
     There is no fallback to try -- the old host is gone, not slow.

     Only the HOST changed. Verified against the new docs rather than assumed:
     path /api/alt-fuel-stations/v1/nearest.json, every parameter below, the
     fuel_stations[] envelope and the ev_charging_units[].connectors{}.power_kw
     shape this function reads are all unchanged, and existing API keys keep
     working -- "only the domain in the URL needs to be updated". */
  ...
  try {
    ...
    if (resp.status === 429) return fail('The shared demo key is used up for now. Add your own free key at the bottom of the Garage tab, or use the built-in list.');
    ...
    S.finder.live = (data.fuel_stations || []).map(f => {
      ...
      return { name: f.station_name, city: f.city, network: f.ev_network,
               networkId: this.nrelNetwork(f.ev_network), kW,
               distance: f.distance, level: 'DC' };
    }).filter(r => r.name);
    S.finder.busy = false;
    if (!S.finder.live.length) S.finder.error = 'No DC fast chargers found within 25 miles.';
    this.renderFinder();
  } catch (e) {
...
renderBuildInfo() {
  ...
  el.innerHTML = `<div style="text-align:center;margin-top:26px;padding-top:18px;
    ...
    <div style="margin-top:16px;text-align:left">
      <label class="fl">Station-lookup key (optional)</label>
      <input type="text" placeholder="DEMO_KEY" value="${h(S.nrelKey || '')}"
             oninput="UI.setNrelKey(this.value)">
      <p class="hint">"Search near me" uses the US Department of Energy's free station
        database. Without a key it shares a demo allowance that runs out quickly.
        A personal key is free from developer.nlr.gov/signup and lifts the limit.
        Everything else in this app works with no key and no signal.</p>
    </div>
  </div>`;
},
